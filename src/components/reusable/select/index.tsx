...
import SelectArrow, { ARROW_WIDTH_SPACE } from './select-arrow';
...
const Select: SelectComponent = ({
	...
	React.useLayoutEffect(() => {
		...
		const maxHeight = Math.min(windowHeight, maxOptionsContainerHeight, 200);
	...
	React.useLayoutEffect(() => {
		...
		maxOptionWidth.current =
			optionsContainerRef.current!.getBoundingClientRect().width +
			(hideArrow ? 0 : ARROW_WIDTH_SPACE);
		...
		>
			...
			<OptionsContainer style={calculateOptionsContainerStyle()} ref={optionsContainerRef}>
				{normalizedOptions.map(option => (
					<OptionElem key={option.id} type="button" onClick={() => handleOptionClick(option)}>
						{withContainer(hasOptionsWidth ? option.optionText : option.labelText)}
					</OptionElem>
				))}
			</OptionsContainer>
			{!hideArrow && <SelectArrow isOpen={Boolean(selectedOption)} />}
		</Root>
	);
};
